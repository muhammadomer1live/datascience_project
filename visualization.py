...
# This Code is used during full operation
# def load_metrics_from_json(filename):
#     """
#     Load metrics data from a JSON file.

#     Args:
#         filename (str): Path to the JSON file containing metrics data.
...
# metrics_data = load_metrics_from_json('metrics-test.json')

# # Extract metrics data from the loaded dictionary
# nic_rouge_scores = metrics_data.get('nic_rouge_scores', {})
# scst_rouge_scores = metrics_data.get('scst_rouge_scores', {})
# sat_rouge_scores = metrics_data.get('sat_rouge_scores', {})

# nic_bleu_score = metrics_data.get('nic_bleu_score', 0)
# scst_bleu_score = metrics_data.get('scst_bleu_score', 0)
# sat_bleu_score = metrics_data.get('sat_bleu_score', 0)

# nic_caption_lengths = metrics_data.get('nic_caption_lengths', [])
# scst_caption_lengths = metrics_data.get('scst_caption_lengths', [])
# sat_caption_lengths = metrics_data.get('sat_caption_lengths', [])

# captions_nic = metrics_data.get('captions_nic', [])
# captions_scst = metrics_data.get('captions_scst', [])
# captions_sat = metrics_data.get('captions_sat', [])

# # Load word embeddings and losses if available
# nic_word_embeddings = np.array(metrics_data.get('nic_word_embeddings', []))
# scst_word_embeddings = np.array(metrics_data.get('scst_word_embeddings', []))
# sat_word_embeddings = np.array(metrics_data.get('sat_word_embeddings', []))

# nic_train_losses = metrics_data.get('nic_train_losses', [])
# nic_val_losses = metrics_data.get('nic_val_losses', [])

# scst_train_losses = metrics_data.get('scst_train_losses', [])
# scst_val_losses = metrics_data.get('scst_val_losses', [])

# sat_train_losses = metrics_data.get('sat_train_losses', [])
# sat_val_losses = metrics_data.get('sat_val_losses', [])

#Testing this code is used,
...
def plot_rouge_and_bleu_scores():
    """
    Create a figure with two subplots to compare ROUGE and BLEU scores.
    ...
    # Plot BLEU Scores for different models
...
def plot_caption_lengths():
    """
    Create a histogram plot of caption lengths for different models.

    Returns:
        matplotlib.figure.Figure: The figure containing the histogram plot.
    """
    fig, ax = plt.subplots(figsize=(12, 6))

    # Plot histograms of caption lengths for NIC, SCST, and SAT
...
def plot_learning_curves():
    """
    Create a plot of training and validation losses over epochs for different models.

    Returns:
        matplotlib.figure.Figure: The figure containing the learning curves plot.
    """
    fig, ax = plt.subplots(figsize=(12, 6))
...
def plot_caption_diversity(captions_list, model_name):
    """
    Create a bar plot of word frequency for captions from a specific model.

    Args:
        captions_list (list of str): List of captions for the model.
        model_name (str): Name of the model to be used in the plot title.

    Returns:
        matplotlib.figure.Figure: The figure containing the word frequency plot.
    """
    word_counts = Counter()
...
def plot_correlation_heatmap():
    """
    Create a heatmap showing the correlation between BLEU and ROUGE scores.
    ...
    data = {
        ...
        'ROUGE-1': [nic_rouge_scores.get('rouge1', 0), scst_rouge_scores.get('rouge1', 0), sat_rouge_scores.get('rouge1', 0)],
        'ROUGE-2': [nic_rouge_scores.get('rouge2', 0), scst_rouge_scores.get('rouge2', 0), sat_rouge_scores.get('rouge2', 0)],
        'ROUGE-L': [nic_rouge_scores.get('rougeL', 0), scst_rouge_scores.get('rougeL', 0), sat_rouge_scores.get('rougeL', 0)]
    }
...
def plot_kmeans_clustering():
    """
    Create a scatter plot of caption lengths colored by KMeans clustering results.

    Returns:
        matplotlib.figure.Figure: The figure containing the KMeans clustering plot.
    """
...
class PlotViewerApp:
    """
    A GUI application for viewing various plots using Tkinter and Matplotlib.

    Attributes:
        root (tk.Tk): The root Tkinter window.
        plot_funcs (list of tuple): List of tuples containing plot names and corresponding plot functions.
        current_plot (int): Index of the currently displayed plot.
    """
    def __init__(self, root):
        """
        Initialize the PlotViewerApp.
        ...
        # List of plot functions and their titles
        self.plot_funcs = [
        ...
        # Setup the Matplotlib figure and canvas for Tkinter
    ...
    def show_plot(self, index):
        """
        Update the displayed plot to the one at the specified index.

        Args:
            index (int): Index of the plot to display.
        """
        self.fig.clear()
